...
#pragma resource "*.dfm"
TFormReconstructionErosion *FormReconstructionErosion;
//---------------------------------------------------------------------------
__fastcall TFormReconstructionErosion::TFormReconstructionErosion(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------

void __fastcall TFormReconstructionErosion::BitBtnMarkerClick(TObject *Sender)
{
...
void __fastcall TFormReconstructionErosion::BitBtnMaskClick(TObject *Sender)
{
TMDIChild *Child=(TMDIChild *)MainForm->ActiveMDIChild;
if(Child!=NULL){
int anchoOriginal=Child->Image->Picture->Bitmap->Width;
...
//---------------------------------------------------------------------------
void __fastcall TFormReconstructionErosion::FormClose(TObject *Sender, TCloseAction &Action)
{
ImageMarker->Picture->Bitmap->Assign(NULL);
ImageMask->Picture->Bitmap->Assign(NULL);
}
//---------------------------------------------------------------------------
void __fastcall TFormReconstructionErosion::BitBtnOKClick(TObject *Sender)
{
...
int iteraciones=0;
while(noestabilidad){
Mascara->Assign(FSEB->Morpho->erodeG(MascaraActual,FSEB->EE,FSEB->ox,FSEB->oy));
Mascara->Assign(FSEB->Morpho->eroRec(Mascara,Marcadora));
...
String name=AnsiString("\\Reconstruction by erosion(")+iteraciones+" iterations)=("+ExtractFileName(ImageMarkerName)+")&("+ExtractFileName(ImageMaskName)+")";
